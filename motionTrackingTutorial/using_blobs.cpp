...
int main()
{
  ...
  if(!image_success){
    cout << endl << "ERROR: img_mat1 failed to be read" << endl;
    exit(1);
  }
  cvtColor(img_mat1, bw_mat1, cv::COLOR_BGR2GRAY);
...
// Mat mat_img(ipl_img );
  ...
  if(!image_success){
    cout << endl << "ERROR: img_mat2 failed to be read" << endl;
    exit(1);
  }
  cvtColor(img_mat2, bw_mat2, cv::COLOR_BGR2GRAY);
  ...
  while (image_success && !quit) {
    ...
    absdiff(bw_mat1, bw_mat2, diff_mat);
    threshold(diff_mat, thrs_mat, 50, 255, cv::THRESH_BINARY);
    blur(thrs_mat, thrs_mat, cv::Size(200, 200));
    threshold(thrs_mat, thrs_mat, 50, 255, cv::THRESH_BINARY);
    ...
    cvFilterByArea(blobs, 500, 1000000); // this one filters blobs by area (limited to range provided)
    cout << "blob 1" << endl;
